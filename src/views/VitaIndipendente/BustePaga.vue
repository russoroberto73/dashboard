  ...
  <div class="relative top-[120px] text-center">
    ...
    <div class="m-5">
      <select v-model="IdAssistente" class="p-3 m-3 rounded-[45px]">
        <template v-for="(assistente, index) in ElencoAssistenti" :key="index">
          <option :value="assistente.Id" v-if="assistente?.Licenziamento === 'vuoto'">
            {{ assistente.Nome + ' ' + assistente.Cognome }}
          </option>
        </template>
      </select>
...
const Items = computed(() => {
  ...
  if (Assistente[0]) {
    ...
    if (Assistente[0].Nome === 'Irina 1') {
      ...
      Assistente[0].Cognome = 'Calancea'
    }
    const IdCategoriaAssistente = Pinia.Categorie().getElenco.find(
      (item) => item.Nome === Assistente[0].Nome
    )
      ? Pinia.Categorie().getElenco.find((item) => item.Nome === Assistente[0].Nome).Id
      : ''
    //if (IdCategoriaAssistente)
    MovimentiAnno = Pinia.Movimenti()
      .getElenco.map((item) => {
        if (
          item.Data.substring(6, 10) === Anno.value.toString() &&
          item.IdCategoria === IdCategoriaAssistente
        ) {
  ...
  return Elenco.map((item) => {
    if (parseInt(Anno.value) === parseInt(item.Anno) && IdAssistente.value === item.IdAssistente) {
      if (PrimaBusta) {
        ...
        FerieAp = Elenco.find(
          (fa) =>
            parseInt(Anno.value) === parseInt(fa.Anno) && IdAssistente.value === fa.IdAssistente
        ).FerieAp
      }
      ...
      const FerieResidue =
        parseFloat(FerieAp) + parseFloat(item.FerieMat, 2) - parseFloat(item.FerieGod, 2)
      return {
        Id: {
          Type: 'string',
          Value: item.Id,
