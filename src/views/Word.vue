<template>
  <div>
    <div
      style="position: fixed; top: 0; left: 500px; color: var(--Testo); font-size: 30px; width: 90%"
    >
      {{ Predizione }}
    </div>
    <div style="text-align: left; margin-top: 110px; margin-left: 200px">
      ...
      <button style="background-color: green" @click="Testo = Backup">Ripristina</button><br />
...
const Onkeyup = () => {
  ...
  const UltimoCarattere = Testo.value.substring(Testo.value.length - 1, Testo.value.length)
  const UltimaParola = ArrayParole[ArrayParole.length - 1]
  //console.log(UltimaParola, UltimoCarattere)
  const CodiceFrase = Frasi.find((item) => item.Codice === UltimaParola)
